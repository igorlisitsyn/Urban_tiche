...
@di.message_handler(text = 'Регистрация')
async def sing_up(message: types.Message):
    await RegistrationState.username.set()
    await message.reply('Введите свое имя:')

@di.message_handler(state=RegistrationState.username)
async def set_username(message: types.Message, state:FSMContext):
    async with state.proxy() as registr:
        registr['username'] = message.text
    await RegistrationState.next()
    await message.reply('Введите свой email:')

@di.message_handler(state=RegistrationState.email)
async def set_email(message: types.Message, state:FSMContext):
    async with state.proxy() as registr:
        registr['email'] = message.text
    await RegistrationState.next()
    await message.reply('Введите свой возраст:')

@di.message_handler(state=RegistrationState.age)
async def set_email(message: types.Message, state:FSMContext):
    async with state.proxy() as registr:
        registr['age'] = message.text
    await add_user(registr['username'], registr['email'], registr['age'])
    status = is_included(registr['username'])
    if status:
        await message.answer("Регистрация успешная")
    else:
